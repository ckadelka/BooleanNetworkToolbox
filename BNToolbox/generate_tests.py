...
assert bf.get_layer_structure()[0]==0,"generate.random_non_canalizing_function failed"
...
assert bf.get_layer_structure()[0]==0 and min(bf.get_edge_effectiveness())>0,"generate.random_non_canalizing_non_degenerated_function failed"


#Check that a function with exact canalizing depth k is generated
bf = generate.random_k_canalizing(n, k, EXACT_DEPTH=True)
assert bf.get_layer_structure()[0]==k,"generate.random_k_canalizing failed"


#Check that a function with minimal canalizing depth k is generated
bf = generate.random_k_canalizing(n, k, EXACT_DEPTH=False)
assert bf.get_layer_structure()[0]>=k,"generate.random_k_canalizing failed"


#All variables in an NCF are conditionally canalizing
bf = generate.random_NCF(n)
...
for w in range(1,2**(n-1),2):
    ...
    test = np.all(np.array(boolean_function.get_layer_structure_from_can_outputs(bf.get_layer_structure()[3])) == np.array(layer_structure))
